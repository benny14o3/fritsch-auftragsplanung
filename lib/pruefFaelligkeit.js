...
// Prüfstufe eines Prüfpunkts - Altdaten ohne Feld gelten als 'laufend'.
function stufeVon(p) {
  return p.stufe || 'laufend';
}
function istLaufend(p) { return stufeVon(p) === 'laufend'; }
function istErstfreigabeStufe(p) { return stufeVon(p) === 'erstfreigabe'; }
function istEndabnahmeStufe(p) { return stufeVon(p) === 'endabnahme'; }
...
function pruefFaelligkeit(order, plp = [], jetzt = new Date()) {
  ...
  return plp
    // Prüfstufe 'erstfreigabe' (z.B. die Vorhaltemaße) wird nur einmal je
    // Auftrag mit der Erstfreigabe geprüft, 'endabnahme' erst am Ende durch die
    // QS - beide haben kein laufendes Intervall.
    .filter(p => (p.typ === 'masspruefung' || p.typ === 'iopruefung') && istLaufend(p))
    .map(p => {
...
// Stand der Endabnahme (Prüfstufe 'endabnahme'): darf nur die QS erfassen und
// erst, wenn die Menge fertig produziert ist. Für eine Teilsendung kann die QS
// sie vorher bewusst durchführen - dafür steht hier 'teilsendungMoeglich'.
//
// Bewusst NUR die gemeldeten Runden, nicht die Schätzung: eine Abnahme darf
// nicht durch eine Hochrechnung freigeschaltet werden.
function endabnahmeStand(order, plp = [], jetzt = new Date()) {
  const punkte = (plp || []).filter(p => (p.typ === 'masspruefung' || p.typ === 'iopruefung') && istEndabnahmeStufe(p));
  const soll = order.gesamtmenge || order.menge || 0;
  const gemeldet = gefertigteStueckzahl(order, jetzt).gemeldet;
  const mengeFertig = soll > 0 && gemeldet >= soll;

  return {
    erforderlich: punkte.length > 0,
    mengeFertig,
    gemeldet,
    soll,
    teilsendungMoeglich: !mengeFertig,
    punkte: punkte.map(p => {
      const letzte = letztePruefung(order, p._id);
      return {
        pruefpunktId: String(p._id),
        bezeichnung: p.bezeichnung,
        typ: p.typ,
        sollwert: p.sollwert ?? null,
        toleranzMin: p.toleranzMin ?? null,
        toleranzMax: p.toleranzMax ?? null,
        einheit: p.einheit || '',
        pruefmittel: p.pruefmittel || '',
        letztePruefung: letzte,
        erledigt: !!letzte,
      };
    }),
